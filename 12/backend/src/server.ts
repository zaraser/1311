...
// --------------------------------------------------------------
function broadcastToUsers(io: SocketIOServer, connected: Map<string, any>, userIds: string[], event: string, payload: any) {
...
async function startServer() {
  ...
  fastify.post("/api/invite", async (req: any) => {
    ...
    const exists = inviteQueries.outgoing
      .all(inviterId)
      .some((inv: any) => inv.userId === inviteeId);

    if (!exists) inviteQueries.create.run(inviterId, inviteeId);
  ...
  // MATCH HISTORY
  fastify.post("/api/matches", async (req: any) => {
    const {
      player1Id,
      player2Id,
      winnerId,
    ...
    if (!player1Id || !player2Id) return { error: "Missing player IDs" };

    matchQueries.create.run(
      player1Id,
      player2Id,
      winnerId || null,
      score || null,
  ...
  fastify.get("/api/matches/:userId", async (req: any) => {
    const { userId } = req.params;
    return { matches: matchQueries.getByUser.all(userId, userId) };
  });

  fastify.get("/api/matches/:userId/regular", async (req: any) => {
    const { userId } = req.params;
    return { matches: matchQueries.getRegular.all(userId, userId) };
  });

  fastify.get("/api/matches/:userId/tournament", async (req: any) => {
    const { userId } = req.params;
    return { matches: matchQueries.getTournament.all(userId, userId) };
  });

  fastify.get("/api/matches/:userId/stats", async (req: any) => {
    const { userId } = req.params;
    const stats = matchQueries.getStats.get(
      userId,
      userId,
      userId,
      userId,
  ...
  fastify.post("/api/tournament/notify", async (req: any) => {
    const { message, status } = req.body;
    if (io) io.emit("tournament_update", { message, status });
    return { success: true };
  });
  ...
  // Правильная конфигурация Socket.IO для WSS
  io = new SocketIOServer(server, {
    cors: {
      origin: "*",
      methods: ["GET", "POST"],
      // НЕ указываем credentials - это ломает WebSocket handshake
    },
    transports: ["websocket", "polling"], // WebSocket + polling fallback
    // НЕ включаем allowEIO3 - клиент и сервер должны работать в EIO4
    pingTimeout: 60000,
    pingInterval: 25000,
  });
  ...
  server.listen(PORT, "0.0.0.0", () => {
    ...
    console.log(`📡 WebSocket available at wss://localhost:${PORT}`);
  });

  // ⛓️ Держим процесс активным, чтобы Docker не убил контейнер
  // Promise никогда не резолвится, поэтому процесс работает бесконечно
  await new Promise(() => {});
}
